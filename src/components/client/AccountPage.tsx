...
export function AuthenticatedAccountPage({ user, token, logout, readOnly = false }: Pick<ReturnType<typeof useAuth>, 'user' | 'logout'> & { token: string; readOnly?: boolean }) {
    const router = useRouter();
    ...
    const [events] = useState(() => readOnly ? {track: (_event: Omit<PortalClientEvent, 'sessionId' | 'sequence'>) => {}} : createPortalEventReporter({ token }));
            ...
            <div className="relative pt-24 md:pt-32 pb-8 md:pb-16 px-4 overflow-hidden z-10">
                ...
                <div className="max-w-6xl mx-auto relative z-10">
                    ...
                    <nav aria-label="Sekcje konta" className="grid grid-cols-1 min-[360px]:grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 bg-zinc-900/50 p-2 rounded-2xl border border-zinc-700 backdrop-blur-xl">
                        <AccountTabButton label="Moje zamówienia" active={activeTab === 'orders'} onClick={() => { if (!readOnly) window.history.replaceState(null, '', '/konto?tab=orders'); setActiveTab('orders'); }} icon={<ShoppingBag className="h-6 w-6" />} />
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                        ...
                        {activeTab === 'preparation' && (
                            <ClientStyleGuidePanel authToken={token}
                            ...
                            {offers.map((offer) => {
                                ...
                                return (
                                    <div key={offer.id} className={`rounded-[2rem] overflow-hidden transition-all hover:scale-[1.01] ${needsAction
                                        ...
                                        <ClientOfferRecommendedAlbums authToken={token} offerId={offer.id} />
                                    </div>
                                );
                            })}
                        </div>
