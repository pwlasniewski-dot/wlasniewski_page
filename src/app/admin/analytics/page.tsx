...
type AnalyticsData = {
  ...
  dataQuality: {
    ...
    ingest: {
      ...
      errorBatches: number;
      reasons: Array<{ reason: string; outcome: string; batches: number; events: number }>;
    };
  };
  financeCoverage: { mode: string; ledgerStartedAt: string | null; note: string };
  summary: {
    users: number;
    sessions: number;
    pageViews: number;
...
export default function AnalyticsPage() {
  ...
  const cards = data ? [
    ...
    { label: 'Utworzone rezerwacje', value: data.summary.bookings, compare: data.comparison.bookingsPct, icon: ShoppingCart },
    { label: 'Konwersja', value: `${data.summary.conversionRate}%`, compare: null, icon: TrendingUp },
    { label: 'Wartość rezerwacji', value: `${data.summary.bookingValue.toLocaleString('pl-PL')} zł`, compare: data.comparison.bookingValuePct, icon: WalletCards },
    { label: 'Zarejestrowane wpłaty netto', value: `${data.summary.receivedPaymentsNet.toLocaleString('pl-PL')} zł`, compare: data.comparison.receivedPaymentsPct, icon: WalletCards },
  ] : [];
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 p-4 md:p-8">
      <div className="max-w-[1600px] mx-auto space-y-6">
        ...
        {data?.financeCoverage && (
          <p className="rounded-xl border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-xs text-amber-100">
            {data.financeCoverage.note}
          </p>
        )}

        {data?.dataQuality.ingest && (
          <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 px-4 py-3 text-xs text-zinc-300">
            <div className="flex flex-wrap gap-x-5 gap-y-2">
              <span>Przyjęte paczki: <strong className="text-emerald-300">{data.dataQuality.ingest.acceptedBatches}</strong></span>
              <span>Odrzucone paczki: <strong className={data.dataQuality.ingest.rejectedBatches ? 'text-amber-300' : 'text-zinc-100'}>{data.dataQuality.ingest.rejectedBatches}</strong></span>
              <span>Błędy zapisu/limitera: <strong className={data.dataQuality.ingest.errorBatches ? 'text-red-300' : 'text-zinc-100'}>{data.dataQuality.ingest.errorBatches}</strong></span>
              <span>Wykluczone: <strong className="text-zinc-100">{data.dataQuality.ingest.excludedBatches}</strong></span>
            </div>
            {(data.dataQuality.ingest.rejectedBatches > 0 || data.dataQuality.ingest.errorBatches > 0) && (
              <p className="mt-2 text-zinc-400">
                Powody: {data.dataQuality.ingest.reasons
                  .filter(row => row.outcome === 'rejected' || row.outcome === 'error')
                  .map(row => `${row.reason}: ${row.batches}`)
                  .join(', ')}
              </p>
            )}
          </div>
