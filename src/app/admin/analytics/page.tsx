...
export default function AnalyticsPage() {
  ...
  useEffect(() => { if (new URLSearchParams(window.location.search).get('view') === 'sales') setActiveTab('sales'); }, []);
  ...
  return <div className="min-w-0 bg-zinc-950 text-zinc-100"><div className="mx-auto min-w-0 max-w-[1600px] space-y-4">
    ...
    <div id="analytics-panel-sales" role="tabpanel" aria-labelledby="analytics-tab-sales" hidden={activeTab !== 'sales'} className="space-y-5">
      <FinanceActuals startDate={start} endDate={end}/>
      {data && rangeMatches && <>
      ...
      <details className="min-w-0 rounded-2xl border border-zinc-700"><summary className="min-h-11 cursor-pointer p-4 font-semibold text-emerald-200">Symulator rentowności — pokaż założenia i obliczenia</summary><PodRevenueSimulator/></details>
    </div>
    {(!data || !rangeMatches) && <><div id="analytics-panel-visits" role="tabpanel" aria-labelledby="analytics-tab-visits" hidden={activeTab !== 'visits'} className="text-sm text-zinc-400">Raport wizyt oczekuje na poprawny odczyt danych.</div><div id="analytics-panel-seo" role="tabpanel" aria-labelledby="analytics-tab-seo" hidden={activeTab !== 'seo'} className="text-sm text-zinc-400">Raport SEO oczekuje na poprawny odczyt danych.</div></>}
    {data && rangeMatches && <>
