...
export default function InviteClient({ initialChallenge, uniqueLink }: { initialChallenge: ChallengeData | null, uniqueLink: string }) {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            const url = `${window.location.origin}/foto-wyzwanie/invite/${uniqueLink}`;
            setShareUrl(url);
            setFacebookShareUrl(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}`);
            setWhatsappUrl(`https://wa.me/?text=${encodeURIComponent(`Hej! ${challenge?.inviter_name} zaprosił mnie na foto-wyzwanie! Chcesz się przyłączyć? ${url}`)}`);
            setMailUrl(`mailto:?subject=Zaproszenie na foto-wyzwanie&body=${encodeURIComponent(`Hej!\n\n${challenge?.inviter_name} zaprosił mnie na foto-wyzwanie fotograficzne! Możesz się przyłączyć tutaj: ${url}\n\nZaproszenie czyni życie bardziej kolorowe!`)}`);
        }
    }, [uniqueLink, challenge?.inviter_name]);
                    ...
                    <div className="bg-gradient-to-br from-zinc-800/50 to-zinc-900/50 rounded-2xl p-8 border border-zinc-700 shadow-2xl mb-8">
                        ...
                        <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4 mb-8">
                            <p className="text-sm text-blue-200">
                                ℹ️ To zaproszenie wygasa za 30 dni. Kliknij "Przyjmij!", aby wybrać datę i godzinę sesji.
                            </p>
                        </div>
