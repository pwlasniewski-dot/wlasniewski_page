...
function isBookingStart(event: ParsedEvent) {
  return event.event_type === 'v2_booking_start'
    ...
    || (event.event_type === 'v2_form_start' && normalizePath(event.page_url).includes('rezerwacja'));
}

function isBookingComplete(event: ParsedEvent) {
  return ['v2_booking_complete', 'v2_booking_completed', 'v2_payment_success'].includes(event.event_type);
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [currentRows, previousRows, bookings, previousBookings] = await Promise.all([
    ...
    const revenue = bookings.reduce((sum, booking) => sum + (booking.price || 0), 0) / 100;
    const previousRevenue = previousBookings.reduce((sum, booking) => sum + (booking.price || 0), 0) / 100;
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        revenue,
        conversionRate: summary.sessions ? Math.round((bookings.length / summary.sessions) * 1000) / 10 : 0,
      },
      comparison: {
        ...
        bookingsPct: pctChange(bookings.length, previousBookings.length),
        revenuePct: pctChange(revenue, previousRevenue),
      },
      timeSeries,
      sources,
      pages,
