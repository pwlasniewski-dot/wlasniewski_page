...
function isBookingStart(event: ParsedEvent) {
  return event.event_type === 'v2_booking_start'
    ...
    || event.event_type === 'v2_aero_inquiry_started';
}

function isBookingComplete(event: ParsedEvent) {
  return ['v2_booking_created', 'v2_booking_complete', 'v2_booking_completed', 'v2_aero_inquiry_submitted'].includes(event.event_type);
}
