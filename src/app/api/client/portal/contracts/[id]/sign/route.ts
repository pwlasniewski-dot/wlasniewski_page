    ...
    try {
        ...
        try {
            ...
            if (adminEmail) {
                await sendEmail({
                    to: adminEmail,
                    subject: `✅ Umowa ${contract.contract_number} PODPISANA — ${clientName}`,
                    html: `
<div style="font-family:Arial,sans-serif;padding:20px;background:#0a0a0a;color:#fff;max-width:600px;margin:0 auto;">
  ...
  <table style="width:100%;border-collapse:collapse;">
    ...
    <tr><td style="padding:8px 0;color:#888;font-size:13px;">Oferta</td><td style="padding:8px 0;color:#fff;font-size:13px;text-align:right;">${contract.offer?.title || 'N/A'}</td></tr>
  ...
  <div style="text-align:center;margin:24px 0;">
    <a href="${appUrl}/admin/generator-umow" style="display:inline-block;background:#c5a059;color:#000;text-decoration:none;padding:14px 32px;border-radius:8px;font-weight:bold;">Zarządzaj umowami →</a>
            ...
            console.error('[Contract Sign] Failed to send admin notification:', emailError);
        }
        ...
        try {
            if (clientEmail) {
                await sendEmail({
                    to: clientEmail,
                    subject: `✅ Umowa ${contract.contract_number} podpisana — dziękujemy!`,
                    html: `
<div style="font-family:Arial,sans-serif;padding:20px;background:#0a0a0a;color:#fff;max-width:600px;margin:0 auto;">
  ...
  <div style="background:#111;border:1px solid #222;border-radius:12px;padding:32px;">
    ...
    <div style="background:#1a1a1a;border:1px solid #333;border-radius:8px;padding:16px;margin:20px 0;">
      ...
      <p style="color:#c5a059;font-size:18px;font-weight:bold;margin:0;font-family:monospace;">${contract.contract_number}</p>
      <p style="color:#555;font-size:12px;margin:6px 0 0;">Oferta: ${contract.offer?.title || 'N/A'}</p>
    </div>
    <div style="text-align:center;margin:24px 0;">
  ...
  <p style="color:#444;font-size:11px;text-align:center;margin:16px 0 0;">© ${new Date().getFullYear()} Przemysław Właśniewski · <a href="https://wlasniewski.pl" style="color:#c5a059;">wlasniewski.pl</a></p>
            ...
            console.error('[Contract Sign] Failed to send client confirmation:', emailError);
        }
