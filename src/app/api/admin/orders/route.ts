...
import { orderPhotoIds } from '@/lib/galleries/order-presentation';
    ...
    try {
        ...
        const allPhotoIds = Array.from(
            new Set([
                ...
                ...photoOrders.flatMap(order => { const metadata=readShopMetadata(order.product_ids); return metadata ? orderPhotoIds(metadata) : []; }),
        ...
        const photoOrdersMapped: UnifiedOrder[] = photoOrders.map((order) => {
            ...
            if (merchandise) return {
                ...
                merchandise,
                photoIds: orderPhotoIds(merchandise),
                selectedPhotos: orderPhotoIds(merchandise).flatMap(id => { const photo = photosById.get(id); return photo ? [photo] : []; }),
            ...
            const selectedPhotos = Array.from(new Set(parsedPhotoIds))
            ...
            const extraPhotoItems = groupExtraMetadata?.lines?.length
                ? Object.entries(
                    groupExtraMetadata.lines.reduce((acc, line) => {
                        const key = line.print_size;
                        if (!acc[key]) {
                            acc[key] = { quantity: 0, unitAmount: line.unit_amount || fallbackExtraPhotoUnitAmount };
                        }
                    ...
                    totalAmount: entry.quantity * entry.unitAmount,
                    sizeLabel: size,
                }))
                : (order.photo_count > 0 ? [{
                    kind: 'extra_photo' as const,
                    ...
                    unitAmount: Math.max(0, groupExtraMetadata?.unit_amount || fallbackExtraPhotoUnitAmount),
                    totalAmount: Math.max(0, (groupExtraMetadata?.unit_amount || fallbackExtraPhotoUnitAmount) * order.photo_count),
                    sizeLabel: groupExtraMetadata?.print_size || undefined,
                }] : []);
            ...
            const extraPhotoUnitAmount = extraPhotoItems[0]?.unitAmount || 0;
