...
import { isProductImageUrl, readProductImages, isProductVideoUrl } from '@/lib/galleries/product-media';
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string; productId: string }> }) {
    return withAuth(request, async () => {
        ...
        try {
            ...
            const { title, description, image_url, preview_images, video_url, sample_pages, price, is_active } = body;
            ...
            if (video_url != null && video_url !== '' && !isProductVideoUrl(video_url)) return NextResponse.json({ error: 'Film musi mieć poprawny adres HTTPS pliku MP4.' }, { status: 400 });
            if (sample_pages !== undefined && (!Array.isArray(sample_pages) || sample_pages.length > 12 || !sample_pages.every(isProductImageUrl))) return NextResponse.json({ error: 'Dodaj maksymalnie 12 zdjęć przykładowych rozkładówek.' }, { status: 400 });
            ...
            const product = await prisma.galleryProduct.update({ where: { id: itemId }, data: { title: title.trim(), description: description || null, image_url: image_url || null, ...(preview_images === undefined ? {} : { preview_images: readProductImages(preview_images) }), ...(video_url === undefined ? {} : {video_url: video_url || null}), ...(sample_pages === undefined ? {} : {sample_pages:readProductImages(sample_pages)}), price, is_active } });
