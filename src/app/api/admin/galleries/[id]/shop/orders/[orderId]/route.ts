...
export async function PATCH(request:NextRequest,{params}:{params:Promise<{id:string;orderId:string}>}) {return withAuth(request,async()=>{try {
 ...
 const statuses=['new','ordered','received','packed','shipped','collected'];
 ...
 if(!stages.includes(body.status)) throw new ShopValidationError('Etap nie pasuje do sposobu odbioru zamówienia.',409);
 const next=stages.indexOf(body.status),previous=stages.indexOf(metadata.fulfillment.status);
 if(next!==previous && next!==previous+1) throw new ShopValidationError('Zapisuj kolejne etapy realizacji zamówienia.',409);
 const tracking=pickup ? null : body.trackingNumber?.trim() || metadata.fulfillment.trackingNumber;
