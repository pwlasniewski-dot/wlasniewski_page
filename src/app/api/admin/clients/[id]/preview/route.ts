...
export async function POST(request:NextRequest,{params}:{params:Promise<{id:string}>}) {
 return withAuth(request,async authenticated => {
  ...
  const blockedReason=!client.is_active ? 'Konto jest nieaktywne — klient nie może się teraz zalogować, ale administrator widzi pełny podgląd zapisanych danych.' : client.password_reset_required ? 'Klient nie ustawił jeszcze hasła — administrator nadal widzi pełny podgląd zapisanych danych.' : null;
  const token=await generateClientPreviewToken({id:client.id,email:client.email},authenticated.user!.id);
