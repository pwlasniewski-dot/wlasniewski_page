...
export async function POST(request: NextRequest) {
  return withAuth(request, async (req) => {
    try {
      ...
      let contract;
      if (offerIdInt) {
        // Create or update the contract (upsert because of unique offer_id constraint)
      ...
      // Fetch full offer object for email logic if we have it
      const offer = offerIdInt ? await prisma.offer.findUnique({
        where: { id: offerIdInt },
        include: { user: true }
      }) : null;

      // Determine client email
      let clientEmail = offer?.user?.email || offer?.client_email;
      let clientName = offer?.user?.name || offer?.client_email || 'Kliencie';
      const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://wlasniewski.pl';

      // If no offer, try to find user by client_id if provided
      if (!clientEmail && data.client_id) {
        const user = await prisma.user.findUnique({ where: { id: data.client_id } });
        if (user) {
          clientEmail = user.email;
          clientName = user.name || 'Kliencie';
        }
      }
  ...
  <div style="max-width:600px;margin:0 auto;padding:40px 20px;">
    ...
    <div style="background:#111;border:1px solid #222;border-radius:12px;padding:40px;margin-bottom:24px;">
      ...
      <p style="color:#ccc;font-size:16px;line-height:1.6;margin:0 0 24px;">
        Twoja umowa jest gotowa. Możesz ją przejrzeć i pobrać w swoim panelu klienta.${contract.pdf_url ? ' Umowa w formacie PDF znajduje się w załączniku.' : ''}
      </p>
      ...
      <div style="background:#1a1a1a;border:1px solid #333;border-radius:8px;padding:20px;margin:24px 0;">
        ...
        ${offer ? `<p style="color:#666;font-size:13px;margin:8px 0 0;">Oferta: ${offer.title}</p>` : ''}
      </div>
