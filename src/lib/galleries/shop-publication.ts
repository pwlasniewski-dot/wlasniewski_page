...
type PublicationProduct={id:number;price:number;is_active:boolean;description?:string|null;image_url?:string|null};
export function productsReadyToPublish(config:ShopConfig,products:PublicationProduct[]) {
 return products.filter(product=>config.publicOffer?.productIds.includes(product.id) &&
...
export function visibleOfferCount(config:ShopConfig,products:PublicationProduct[]) {
 ...
 return products.filter(product=>config.publicOffer!.productIds.includes(product.id) && product.is_active && product.price>0 &&
