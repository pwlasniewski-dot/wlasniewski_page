...
### 2.1. Homepage Editor (Zaawansowany Kreator Witryny)
Najbardziej złożony moduł panelu, pozwalający na dynamiczne budowanie strony głównej bez ingerencji w kod.
...
- **Features Editor**: Lista atutów w formie kafli z ikonami i opisami.
...
- **Info Band Editor**: Bloki informacyjne (np. "Dlaczego fotografia dronowa?"). Wsparcie dla różnych układów graficznych.
...
- **Flow**: Klient otrzymuje/kupuje pakiet wyzwania -> Wysyła zaproszenie do wybranej osoby -> System generuje unikalny link -> Zaproszony wchodzi na stronę z dedykowanym widokiem (wyzwanie zaakceptowane/odrzucone).
- **Statusy**: `sent`, `viewed`, `accepted`, `scheduled`, `completed`, `expired`.
...
3. Automatycznie kopiuje wybrane URL do schowka administratora.
4. Pozwala na tagowanie i filtrowanie zasobów.

### 10.2. Obsługa Wideo w HeroSlider
- Formaty: .mp4, .webm (z kodekiem H.264/H.265).
- Parametry: `muted`, `loop`, `playsInline` (kluczowe dla autoodtwarzania na iOS/Android).
- Fallback: System wymaga miniatury obrazu, która jest wyświetlana przed załadowaniem wideo.
