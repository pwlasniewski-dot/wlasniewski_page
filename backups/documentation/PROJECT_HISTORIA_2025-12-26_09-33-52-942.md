...
- ✅ **Client Login & Portal**: Wdrożono stronę `/foto-wyzwanie/login` oraz mechanizm autoryzacji sesji dla klientów wyzwań.
...
- ✅ Log recent Photo Challenge UX and compliance improvements.
- ✅ Added booking deletion (Trash icon) in admin panel.
- ✅ Fixed 404 error for `/admin/bookings/orders` by moving the gift card orders page to the correct location.
- ✅ Implemented `DELETE` method in bookings API.
