...
let accountBuyer=null,lastPayuInput=null;
...
Module._load=function(request,parent,isMain){if(request==='./order-account')return {orderClient:async()=>accountBuyer};if(request==='@/lib/shipping/inpost-point')return {verifyParcelPoint:async()=>{}};if(request==='@/lib/email/sender')return {sendEmail:async()=>{mails++},getAdminEmail:async()=> 'admin@example.com'};if(request==='@/lib/auth/middleware')return {withAuth:async(req,fn)=>adminAllowed?fn():require('next/server').NextResponse.json({error:'Unauthorized'},{status:401})};if(request==='@/lib/db/prisma')return {__esModule:true,default:db};if(request==='./individual-access')return {authorizeIndividualGallery:async()=>({allowed:access})};if(request==='@/lib/auth/parent-jwt')return {extractTokenFromHeader:x=>x,verifyParentToken:async()=>({participant_id:participant,gallery_id:12})};if(request==='@/lib/payu')return {createPayUOrder:async input=>{lastPayuInput=input;payuCalls++;return {orderId:'pay'+payuCalls,redirectUri:'https://payments.example/order'}},extractClientIpv4:()=> '127.0.0.1'};return load.apply(this,arguments)};
...
await check('authenticated owner returns to account order; group and shared checkout retain their own flow',async()=>{
 settings.set('gallery_shop_12',JSON.stringify(config));gallery.client_id=7;accountBuyer={id:7,email:'owner@example.com'};
 const result=await postShopOrder(req(body,'account-return-order-0001'),{accessCode:'private'});assert.equal(result.status,200);
 assert.equal(new URL(lastPayuInput.continueUrl).pathname,'/konto');assert.equal(new URL(lastPayuInput.continueUrl).searchParams.get('tab'),'orders');assert.equal(JSON.parse(rows.at(-1).product_ids).customerId,7);
 accountBuyer=null;
 const guest=await postShopOrder(req(body,'guest-return-order-00001'),{accessCode:'private'});assert.equal(guest.status,200);
 assert.equal(new URL(lastPayuInput.continueUrl).pathname,'/galeria/private');assert.equal(JSON.parse(rows.at(-1).product_ids).customerId,undefined);
});
