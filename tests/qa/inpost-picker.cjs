...
(async () => {
  ...
  await check('InPost: actual filters, bounded output and unavailable-point rejection', async () => {
    ...
    const url = new URL(calls.at(-1).url); assert.equal(url.searchParams.get('post_code'), '87-100'); assert.equal(url.searchParams.get('functions'), 'parcel_collect'); assert.equal(url.searchParams.get('per_page'), '20'); assert.equal(calls.at(-1).options.headers.Authorization, undefined);
  ...
  console.log('InPost picker: 5 groups PASS');
